...
for(t in 1:maxt){
  ...
  if(t==1){
    ...
    Smat[[t]] = predict_bart(mod[[ct]],dat.pred, Gcomp = TRUE)$samp_y
    ...
    Smat[[t]] = predict_bart(Smod[[sct]],dat.pred, Gcomp = TRUE)$samp_y
    sct = sct + 1
    
    fitdat1 =  fitdat[get(theY_ind_lag1)==1,]
    ytab = table(fitdat1$outcome)
    ylevels = as.numeric(names(ytab)[ytab > 0])
    tmp1 = predict_bart(Smod[[sct]],dat.pred, Gcomp = TRUE)$samp_y
    sct = sct + 1
...
png(paste0("C:\\Users\\Yizhen Xu\\Google Drive\\Desktop\\2015 summer\\HIV Treatment Cascade\\Draft\\paper2Draft9\\Accuracy Comparison - Diag Plots\\07-13\\CondSSimEngA",A,".png"),width = 600, height = 400)
...
png(paste0("C:\\Users\\Yizhen Xu\\Google Drive\\Desktop\\2015 summer\\HIV Treatment Cascade\\reports\\Exploratory\\DebugA",A,".png"),width = 600, height = 400)
barplot(counts,
