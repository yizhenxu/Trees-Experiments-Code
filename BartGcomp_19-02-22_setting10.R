...
varsel = function(cut, IP){
  IP = sort(IP) 
  res = names(IP[which(IP >= cut)])
  return(res)
}
######################################################################################################################
######################################################################################################################
...
vsc = 0.03 #variable selection cut
...
# !!!!!!!!!!!! only look at engagement model!!!!!!!!
for(prevS in c(1)){ #### S_{t-1} = 0 - disengaged, 1 - engaged
  ...
  Xname[[1]] = RHS
  Xname[[2]] = varsel(vsc, Bmod7[[2]]$Inclusion_Proportions[[2]])
  Xname[[3]] = varsel(vsc, Bmod7[[2]]$Inclusion_Proportions[[3]])
...
res[2:4,] = matrix(unlist(lapply(1:4, function(s) summ(PM1,s,1))), ncol=4)
