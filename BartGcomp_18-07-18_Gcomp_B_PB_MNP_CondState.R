...
for(t in 1:maxt){
  ...
  if(t==1){
    ...
    Smat[[t]] = MNPpredict(m = mod[[ct]], Xmat = dat.pred, nsub, basey1, c(1,2,4), doparallel = parell)
    ...
    Smat[[t]] = MNPpredict(m = Smod[[sct]], Xmat = dat.pred, nsub, basey[sct], ylevels, doparallel = parell)# basey is disengagement
    sct = sct + 1
    
    fitdat1 =  fitdat[get(theY_ind_lag1)==1,]
    ytab = table(fitdat1$outcome)
    ylevels = as.numeric(names(ytab)[ytab > 0])
    tmp1 = MNPpredict(m = Smod[[sct]], Xmat = dat.pred, nsub, basey[sct], ylevels, doparallel = parell)# basey is disengagement
    sct = sct + 1
