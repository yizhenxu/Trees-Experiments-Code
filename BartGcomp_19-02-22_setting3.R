...
for(prevS in c(0,1)){ #### S_{t-1} = 0 - disengaged, 1 - engaged
  ...
  ntL[[ind]] = 50#c(100,100,100),c(50,50,50),c(100,50,50)
  ...
  if(doit){
    ...
    ## diagnostic for all 
    Bmod2[[ind]] = model_bart(formula = as.formula(fml), data = d, type = "multinomial",
                              base = baseyl[ind],
...
save(fml,baseyl,ntL, Bmod2, file = paste0("Mod3Run",setting,".RData"))
